...
class TransactionService:
    ...
    def create_transaction(self, transaction_data: TransactionCreate) -> Transaction:
        ...
        Raises:
            ...
            transaction category doesn't exist, or property doesn't exist
        """
        # Validate transaction category exists
        category = transaction_category_service.get_transaction_category(
            transaction_data.transaction_category_id
        )
        if category is None:
            raise ValueError(
                f"Transaction category with "
                f"ID '{transaction_data.transaction_category_id}' does not exist"
            )
        ...
        if transaction_data.property_id is not None:
            ...
            if property_obj is None:
                raise ValueError(
                    f"Property with ID '{transaction_data.property_id}' does not exist"
                )
        ...
        transaction_dict.update({"date_created": now})
        ...
        try:
            ...
            if transaction_data.transaction_category_id is not None:
                ...
                if category is None:
                    raise ValueError(
                        f"Transaction category with "
                        f"ID '{transaction_data.transaction_category_id}' "
                        f"does not exist"
                    )
            ...
            if transaction_data.property_id is not None:
                ...
                if property_obj is None:
                    raise ValueError(
                        f"Property with "
                        f"ID '{transaction_data.property_id}' does not exist"
                    )

            # Prepare update data (exclude None values)
            update_dict = {
                k: v for k, v in transaction_data.model_dump().items() if v is not None
            }

            # Ensure datetime objects are properly handled
            if "transaction_date" in update_dict and isinstance(
                update_dict["transaction_date"], datetime
            ):
